    ...
    >
      ...
      <main className="flex justify-center items-center relative h-full">
        <section className="flex flex-col justify-between items-center relative h-full p-5 sm:p-1 w-full">
         { selectedFile 
            ?
            <div className="flex-grow gap-3 flex flex-col justify-center items-center p-2">
              <img src={selectedFile} alt="" className="w-full max-h-[350px] object-cover transition"/> 
              <TrashSimple  onClick={() => setSelectedFile(null)} className="post-buttons hover:text-red-600 hover:bg-red-200 hover:scale-110 transition-all ease-in-out"/>
            </div> 
            : 
            (
