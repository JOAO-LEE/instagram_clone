...
export default function Story({ username, image, isUser }: UserStoryDTO) {
    return (
        <div className="relative group  max-w-fit text-center">
            <div className="min-w-14 bg-gradient-to-tl from-fuchsia-600  via-red-500 to-yellow-300 rounded-full p-[2px] group-hover:scale-110 transition-transform duration-200 ease-out text-center">
                { image ? <img src={image} alt="`${username} profile photo`" className="min-h-14 min-w-14 object-cover rounded-full border-1"/> : <User  weight="thin" className="min-h-14 min-w-14 rounded-full  border-1 bg-white" /> }
            </div>
